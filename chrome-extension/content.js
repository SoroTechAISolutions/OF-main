...
(function() {
   ...
   */
  function syncMessagesToDashboard(messages) {
    const chatPartner = getChatPartnerName();
    const chatUrl = window.location.href;

    log(`Syncing ${messages.length} messages to Dashboard...`);

    // Format messages for sync
    const formattedMessages = messages.map(msg => ({
      text: msg.text,
      isFromCreator: msg.type === 'outgoing',
      timestamp: msg.timestamp,
      hasMedia: msg.hasMedia || false,
      isPPV: msg.isPPV || false
    }));

    sendToBackground('sync_messages', {
      chatUrl,
      fanUsername: chatPartner,
      fanDisplayName: chatPartner,
  ...
  function init() {
    ...
    const waitForChat = setInterval(() => {
      ...
      if (container) {
        ...
        // Sync messages to backend for Dashboard
        syncMessagesToDashboard(messages);

        // AUTO-GENERATE: If last message is from fan and no reply yet, generate AI response
        const lastMessage = messages[messages.length - 1];
        if (lastMessage && lastMessage.type === 'incoming') {
          log('Last message is from fan - requesting AI response...');
          sendToBackground('new_fan_message', {
            ...
            autoGenerated: true // Flag to indicate this is auto-generated on page load
          });
        }
